...
if __name__ == "__main__":
    ...
    parser.add_argument("-sub", "--substrate", type=str, default="cyclopropanation_styrene_EDA")
    parser.add_argument("-rf", "--relax_folder", type=str, default="FastDesign_Relax")
    parser.add_argument("-t", "--temperature", type=float, default=3)
    parser.add_argument("-rank", "--rank_fitness", type=str, choices=["m", "p", "f", "b", "s", "e", "d"], default="s")
    ...
    #         args.baseline_variant_path, preferred_stereoisomer, temperature=temperature, \
    ...
    for row, enz_var_xls_row_info in enumerate(scores_dict.items()):
        ...
        write_xls_row(row + 1, enz_var, xls_row_info, sum_sheets, scores_sheets, ts_sheets, \
                green_style, orange_style)
    workbook.save(args.directory + suffix + ".xls")
